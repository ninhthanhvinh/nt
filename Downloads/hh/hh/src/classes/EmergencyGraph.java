...
public class EmergencyGraph extends Graph {
	...
	public EmergencyGraph(int width, int height, ArrayList<ArrayList<ArrayList<Position>>> adjacencyList,
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				for (int k = 0; k < adjacencyList.get(i).get(j).size(); k++) {
					...
					this.nodes[i][j].setNeighbor(this.virtualNodes[(int) adjacencyNode.x][(int) adjacencyNode.y]);
					this.virtualNodes[i][j].setNeighbor(this.virtualNodes[(int) adjacencyNode.x][(int) adjacencyNode.y]);
					this.virtualNodes[i][j].setNeighbor(this.nodes[(int) adjacencyNode.x][(int) adjacencyNode.y]);
				}
			}
		}
		...
		for (int j = 0; j < this.width; j++) {
			for (int k = 0; k < this.height; k++) {
				...
				for (int i = 0; i < this.agents.size(); i++) {
					double dist = Math.sqrt((x - this.agents.get(i).x) * (x - this.agents.get(i).x)
							+ (y - this.agents.get(i).y) * (y - this.agents.get(i).y));
	...
	public ArrayList<Node2D> calPathAStar(Node2D start, Node2D end) {
		...
		while (openSet.size() > 0) {
			...
			ArrayList<Node2D> neighbors = new ArrayList<>(List.of(current.nodeN, current.nodeE, current.nodeS,
